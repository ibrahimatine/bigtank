...
export class EmailService {
  ...
  constructor(private configService: ConfigService) {
    ...
    if (apiKey) {
      ...
      console.log('[email-service] Resend initialise avec succes');
    } else {
      console.warn('[email-service] RESEND_API_KEY non definie — emails desactives');
    }
  }

  async send(options: EmailOptions): Promise<boolean> {
    if (!this.resend) {
      console.log(`[email-service] Email non envoye (Resend non configure): ${options.subject}`);
      return false;
    }
    ...
    try {
      ...
      console.log(`[email-service] Email envoye a ${options.to}: ${options.subject}`);
      return true;
    } catch (err) {
      console.error('[email-service] Erreur envoi email:', err);
      return false;
    }
  }
